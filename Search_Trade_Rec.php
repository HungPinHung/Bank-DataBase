<?php
    ...
    $COOKIE_UUID = $_COOKIE["UUID"]; 
    
    $SQL_COMMAND = "SELECT `交易資料`.* FROM `交易資料` join `帳戶資料`on `交易資料`.`轉出號碼`=`帳戶資料`.`UUID` OR `交易資料`.`交易號碼`=`帳戶資料`.`UUID` where `帳戶資料`.`UUID`='".$USER_UUID."' and `帳戶資料`.`A_UUID`=(SELECT DISTINCT `UUID` FROM `Bank_Account_Info`where UUID='".$COOKIE_UUID."')";
